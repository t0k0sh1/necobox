...
export function ShellCheatsheet() {
  ...
  const groupedCommands = useMemo(() => getShellCommandsByCategory(), []);

  const filteredGroups = useMemo(() => {
    if (!searchQuery.trim()) return groupedCommands;

    const query = searchQuery.toLowerCase().trim();
    const filtered = new Map<ShellCommandCategory, ShellCommand[]>();
  ...
  const toggleCategory = (category: ShellCommandCategory) => {
  ...
  const getName = (cmd: ShellCommand) =>
    locale === "ja" ? cmd.nameJa : cmd.nameEn;
  const getDescription = (cmd: ShellCommand) =>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <Input
          type="text"
          placeholder={t("shellCommands.searchPlaceholder")}
          value={searchQuery}
      ...
      {!hasResults && (
        <div className="text-center py-8 text-gray-500 dark:text-gray-400">
          {t("shellCommands.noResults")}
        </div>
      )}

      {/* カテゴリごとのリスト */}
      {SHELL_CATEGORY_ORDER.map((category) => {
        ...
        return (
          ...
          >
            ...
            <button
              ...
              aria-label={t(`shellCommands.categories.${category}` as Parameters<typeof t>[0])}
            ...
            >
              <div className="flex items-center gap-2">
                ...
                >
                  {t(
                    `shellCommands.categories.${category}` as Parameters<typeof t>[0]
                  )}
                </span>
              </div>
                        ...
                        <Button
                          ...
                          aria-label={
                            copiedId === `row-${cmd.command}`
                              ? `${t("shellCommands.copied")} ${cmd.command}`
                              : `${t("shellCommands.copy")} ${cmd.command}`
                          }
                        ...
                        <div className="px-4 pb-3 pt-1 bg-gray-50/50 dark:bg-gray-900/20">
                          <div className="ml-6 space-y-1">
                            <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">
                              {t("shellCommands.options")} ({cmd.options.length})
                            </div>
                            {cmd.options.map((opt) => (
                              <div
                                key={opt.flag}
